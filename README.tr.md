...
🇬🇧 Tüm eşleşen İngilizce/Türkçe sayfalar için [doküman indeksine](docs/README.md)
bakın.
...
- **Feed'de video oto-oynat** — web'de `![](…mp4/webm/mov)` artık `<video>` render ediliyor (sanitizer izni + src yine /media-kısıtlı; eskiden kırık `<img>`'di) ve görünür video **sessiz oto-oynar / kayınca durur** (web IntersectionObserver + mobil FlatList viewability); DM'de tap-to-play korunur
- **Video sekmesi (Reels-tarzı)** — dikey **tam-ekran kaydırmalı** akış (`GET /feed/videos` — klip taşıyan public postlar, viewer-aware, keyset-paginate). Görünür klip **sessiz oto-oynar/döner**, **dokun → ses**; yazar + açıklama + 🐢-reaksiyon/yorum bindirmesi. Web (`/videos`, CSS scroll-snap + IntersectionObserver, sidebar + alt-bar linki) + mobil ("Video" sekmesi, sayfalı FlatList, pencerelenmiş expo-video). Paylaşılan klipler **≤2 dk** (client süre kontrolü) + ≤64 MB
